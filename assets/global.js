...
class SliderComponent extends HTMLElement {
  ...
  update() {
    ...
    // Update progress bar
    const sliderCounterCurrent = document.querySelector('.slider-counter--current');
    const productsToDisplay = document.querySelector('.slider-counter--total');
    const progressBar = document.querySelector('.progress-bar');
    
    function updateProgressBar() {
      const progress = (parseInt(sliderCounterCurrent.innerText) / parseInt(productsToDisplay.innerText)) * 100;
...
if (document.querySelector('.contact__field-error')) {
  var fields = document.querySelectorAll('.field--with-error');
  for (var i = 0; i < fields.length; i++) {
    fields[i].querySelector('input').style.border = '.1rem solid var(--state-critical)';
  }
}
...
function setOpenAttribute() {
  if (window.innerWidth <= 989) {
    headerSearch.setAttribute('open', 'true');
  } else {
...
const inputElement = document.getElementsByClassName('field__input');
if (inputElement.value) {
  console.log('Input has been filled');
} else {
  console.log('Input has not been filled');
}
